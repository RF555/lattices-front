...
export function TodoExpandButton({ hasChildren, isExpanded, onToggle }: TodoExpandButtonProps) {
  ...
  return (
    <Tooltip content={isExpanded ? t('actions.collapse') : t('actions.expand')}>
      <button
        type="button"
        onClick={(e) => {
          e.stopPropagation();
      ...
      >
        <ChevronRight
          className={cn('w-3 h-3 transition-transform rtl:-scale-x-100', isExpanded && 'rotate-90')}
        />
      </button>
    </Tooltip>
  );
